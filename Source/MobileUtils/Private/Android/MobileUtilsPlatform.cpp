...
jmethodID FMobileUtilsPlatform::SelectPhotoTextureMethod;
...
FMobileUtilsPlatform::FMobileUtilsPlatform()
{
	...
	{
		if (!(CheckInternetConnectionMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_CheckInternetConnection", "()Z", false)))
		{
			VLOG(LogMobileUtils,"JMETHOD not found!");
		}
		
		if (!(CheckGooglePlayServicesMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_CheckGooglePlayServices", "()Z", false)))
		{
			VLOG(LogMobileUtils,"JMETHOD not found!");
		}
		
		if (!(GetPersistentUniqueDeviceIdMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_GetPersistentUniqueDeviceId", "()Ljava/lang/String;", false)))
		{
			VLOG(LogMobileUtils,"JMETHOD not found!");
		}
		
		if (!(SelectPhotoTextureMethod = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_selectImage", "(I)V", false)))
		{
			VLOG(LogMobileUtils,"JMETHOD not found!");
		}
	...
	{
		VLOG(LogMobileUtils,"GetJavaEnv() returning null!");
	}
}
...
/*
 * SelectPhotoTexture stuff
 */
void FMobileUtilsPlatform::SelectPhotoTexture(int32 CameraOrFile)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, FMobileUtilsPlatform::SelectPhotoTextureMethod, CameraOrFile);
	}
}

/*
extern "C" bool Java_com_epicgames_ue4_GameActivity_nativeOnSelectPhoto(JNIEnv* LocalJNIEnv, jobject LocalThiz, jint Width, jint Height, jbyteArray data)
{
	UE_LOG(LogMobileUtils,Log,TEXT("Java_com_epicgames_ue4_GameActivity_nativeOnSelectPhoto called"));
	//get the new frame
